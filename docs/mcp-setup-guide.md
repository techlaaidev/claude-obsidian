...
|--------|---------|----------|
| **Local REST API** | Exposes vault via HTTPS on port 27124 (self-signed cert) | Yes |
...
- API key is stored in `~/.claude.json` (user home, not committed to git) after `claude mcp add-json --scope user`
- Local REST API only listens on `127.0.0.1` (localhost)
- Do not expose port 27124 to network or reverse proxy it
- RSA private key + cert are generated by the plugin into `.obsidian/plugins/obsidian-local-rest-api/data.json` — this path is gitignored, do NOT commit it
